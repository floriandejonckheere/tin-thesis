...
% TODO: image for replication types
The partitioning method goes hand in hand with the replication capabilities of the data store.
Replication types are usually characterised and divided based upon the amount of \textit{masters} and \textit{slaves} in the cluster.
A master is a server that is the authoritative source for data, while a slave is a node that is dependent on the master for certain queries.
The intention of this system is that when one or more hosts go down, the cluster can still continue to handle requests, albeit not every single one depending on what node went down.
For example, Neo4j's slave nodes in a cluster can only handle read requests, all write requests are forwarded to the master node.
A master failure in this setup renders the cluster at least read-only, and potentially only able to access part of the data that wass stored on the slave nodes.
...
This type of capability is commonly seen key-value stores.
